...
async function backfill() {
  ...
  for (const appointment of appointments) {
    if (appointment.referenceCode) used.add(appointment.referenceCode)
  }

  for (const appointment of appointments) {
    if (appointment.referenceCode) continue
    let seq = 1
    ...
    const sameDate = appointments.filter(
      (a) => a.appointmentDate === appointment.appointmentDate && a.referenceCode,
    )
