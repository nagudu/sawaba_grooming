...
- Vite dev server: port **5174** (`http://localhost:5174`, IPv6 `::1`) — 5173 was occupied by a stale listener, Vite auto-selected 5174. Detached via `Start-Process node.exe node_modules/vite/bin/vite.js` (the `npm.cmd` shim hangs under Start-Process on this machine).
- Backend API: `npm run dev` from `backend/` (tsx watch; the listening pid changes on every code edit — check `netstat -ano | findstr :5000`). Images now live on the backend: `backend/uploads/` served at `/uploads/...` (Vite proxies it), seeded copies come from `backend/seed-assets/` via `npm run db:seed`.
